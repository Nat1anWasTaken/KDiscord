...
class Accuse(Modal):
    ...
    async def callback(self, interaction):
        embed = discord.Embed(title="新的案件", color=discord.Colors.red()) # Initialization the embed

        # Find defendants
        defendants = await utils.find_user_by_name_and_tag(self.children[0].value, interaction.guild,
                                                           multiple=True)
        embed.add_field(name="被告人", value=defendants.join(", "))

        # Find assignee
        assignee = utils.find_user_by_name_and_tag(self.children[1].value, interaction.guild)
